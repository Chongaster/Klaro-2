...
import { showModal, hideModal, applyTheme, updateAuthUI, setMode } from './ui.js';
...
export function initAuth() {
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            setMode(startupMode);
            hideModal();
            setupRealtimeListeners(); // Démarrer l'écoute des données
        } else {
            state.userId = null; state.userEmail = null; state.isAdmin = false;
            detachAllListeners(); // Arrêter l'écoute
            applyTheme('light');
            updateAuthUI(null);
            showAuthModal();
        }
